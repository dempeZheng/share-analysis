 ...
 */
public class RetentionDaily extends BaseAction {
    ...
    private JSONArray sumDailyReport(String _appkey, Map<String, List<String>> params) {
        ...
        String key = "retention"+Constants.SPLIT+_appkey + Constants.SPLIT + _version + Constants.SPLIT + _channel +
                Constants.SPLIT + _create_date + Constants.SPLIT + "*";
        ...
        try {
            ...
            while (it.hasNext()) {
                ...
                map.put("appver", cols[2]);
                map.put("chanelname", cols[3]);
                map.put("create_date", cols[4]);
                map.put("freq", cols[5]);
                Response<List<String>> fieldValueResponse = pipeline.hmget(hkey, "d","w","m");
                pipeline.sync();
                List<String> fieldValues = fieldValueResponse.get();
                String d = fieldValues.get(0);
                String w = fieldValues.get(1);
                String m = fieldValues.get(2);
                map.put("d", d);
                map.put("w", w);
                map.put("m", m);
                jsonArray.add(map);
